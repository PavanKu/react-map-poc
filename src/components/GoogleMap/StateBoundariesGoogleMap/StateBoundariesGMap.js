...
const StateBoundariesGMap = () => {
  ...
  useEffect(() => {
    if (mapInstance) {
      ...
      mapInstance.data.loadGeoJson("/india-states.geojson");
      mapInstance.data.setStyle({
        fillColor: "green",
        strokeWeight: 0,
      });

      mapInstance.data.addListener("mouseover", function (event) {
        mapInstance.data.overrideStyle(event.feature, {
          fillColor: "red",
        });
      });
      mapInstance.data.addListener("mouseout", function (event) {
        mapInstance.data.overrideStyle(event.feature, {
          fillColor: "green",
        });
      });
...
export default StateBoundariesGMap;
